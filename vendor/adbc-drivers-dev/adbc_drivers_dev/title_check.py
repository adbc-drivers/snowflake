...
# From https://github.com/apache/arrow-adbc/blob/main/dev/adbc_dev/title_check.py
# Commit d9dad703bbaf499406f548276d46fad08b292144
# Modified to work with pygit2
...
AllowedComponents = typing.Callable[[str], bool]


def any_components() -> AllowedComponents:
    return lambda _: True


def path_components(root: Path) -> AllowedComponents:
    def _pred(component: str) -> bool:
        # Check if the component is a valid file or directory in the repo
        path = root / component
        return path.is_dir()
...
def matches_commit_format(
    allowed_component: AllowedComponents, title: str
) -> list[str]:
    ...
    if components is not None:
        if not components.strip():
            ...
            for component in components:
                ...
                elif not allowed_component(component):
                    reasons.append(
...
def main():
    ...
    commit = matches_commit_format(path_components(args.root), args.title)
