...
# C# Snowflake ADBC Driver — Tests

Tests for the C# Snowflake ADBC driver (`AdbcDrivers.Snowflake`). The
...
The C# driver tests need only valid **credentials + a warehouse**, plus a **writable**
...
the C# driver tests — they remain only for Interop-config compatibility.
...
both drivers (the C# driver just ignores `driverPath`/`driverEntryPoint`).
