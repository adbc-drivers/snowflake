...
namespace AdbcDrivers.Snowflake.Tests.Integration;

/// <summary>
/// Query throughput benchmark for the C# Snowflake driver. Mirrors the Interop
...
/// test and the <c>[CSHARP]</c>/<c>[INTEROP]</c> log label.
///
...
public class BenchmarkTests
{
    ...
    public async Task BaselineQueryPerformance(int limit)
    {
        ...
        stopwatch.Stop();
        _output.WriteLine($"[CSHARP] Fetched {totalRows} rows in {batchCount} batches with {stream.Schema.FieldsList.Count} columns for limit {limit} in {stopwatch.Elapsed.TotalMilliseconds:F0} ms.");
