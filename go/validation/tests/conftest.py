...
def pytest_addoption(parser):
    ...
    parser.addoption("--vendor-version", action="store", default="10")


@pytest.fixture(scope="session")
def driver(request, pytestconfig) -> adbc_drivers_validation.model.DriverQuirks:
    driver = request.param
    assert driver.startswith("snowflake")
    return snowflake.get_quirks(pytestconfig.getoption("vendor_version"))
