...
func (suite *SnowflakeTests) TestSqlIngestLargeListType() {
	suite.Require().NoError(suite.Quirks.DropTable(suite.cnxn, "bulk_ingest_large_list"))
	...
	sc := arrow.NewSchema([]arrow.Field{
		...
		{
			Name: "col_list", Type: arrow.LargeListOf(arrow.BinaryTypes.String),
			Nullable: true,
		},
	}, nil)
	...
	listbldr := bldr.Field(1).(*array.LargeListBuilder)
	listvalbldr := listbldr.ValueBuilder().(*array.StringBuilder)
	listbldr.Append(true)
	listvalbldr.Append("one")
	listbldr.Append(true)
	...
	suite.Require().NoError(suite.stmt.Bind(suite.ctx, rec))
	suite.Require().NoError(suite.stmt.SetOption(suite.ctx, adbc.OptionKeyIngestTargetTable, "bulk_ingest_large_list"))
	n, err := suite.stmt.ExecuteUpdate(suite.ctx)
	suite.Require().NoError(err)
	suite.EqualValues(3, n)

	suite.Require().NoError(suite.stmt.SetSqlQuery(suite.ctx, `SELECT * FROM "bulk_ingest_large_list" ORDER BY "col_int64" ASC`))
	rdr, n, err := suite.stmt.ExecuteQuery(suite.ctx)
	suite.Require().NoError(err)
	defer rdr.Release()
